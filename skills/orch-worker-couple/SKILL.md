---
name: orch-worker-couple
description: Worker-Checker Couple 架构——将复杂任务拆分为"单一重复+单一能力+文件交接"的 Worker-Checker 对，Orch 只做翻译和观察不介入执行，loop.py 纯代码调度，多层防作弊。触发词：couple、worker-couple、拆任务、并行Worker、多Worker编排、文件交接、Worker写文件
---

# Worker-Checker Couple · 入口
...
```
question: "你在哪个平台上使用 orch-worker-couple？"
options:
