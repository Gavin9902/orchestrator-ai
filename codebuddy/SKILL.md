---
name: orch-worker-couple-codebuddy
description: Worker-Checker Couple 架构（CodeBuddy 版）——将复杂任务拆分为 Worker-Checker 对。硬编码 Task(mode="acceptEdits") 平台映射。触发词：couple、worker-couple、拆任务、并行Worker、文件交接
---

# Worker-Checker Couple · CodeBuddy 版

> **本文件是 orch-worker-couple 的 CodeBuddy 平台专用版本。**
